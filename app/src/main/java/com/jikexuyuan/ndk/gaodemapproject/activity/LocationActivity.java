...
import com.jikexuyuan.ndk.gaodemapproject.Util.Utils;
		...
		public void onLocationChanged(AMapLocation loc) {
			if (null != loc) {
				...
				if (loc.getLatitude() != 0.0) {
					Log.e(TAG, "getIntent: Longitude is\t" + loc.getLongitude() + "Latitude is\t" + loc.getLatitude());
					startActivity(new Intent(LocationActivity.this,NaviActivity.class).putExtra("Longitude",loc.getLongitude()).putExtra("Latitude",loc.getLatitude()));
					stopLocation();
					finish();
				}
			} else {
				tvResult.setText(R.string.location_failure);
			}
		}
	};
